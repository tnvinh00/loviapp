...
const SearchProfileScreen = ({ navigation }) => {
    const { user } = useContext(AuthContext);
    const [name, setName] = useState(null);
    const [listuser, setListuser] = useState([]);
    const [loading, setLoading] = useState(false);
    const [notification, setNotification] = useState("Vui lòng nhập tên bạn muốn tìm kiếm");
    ...
    const handleSearch = async (namee) => {
        ...
        if (namee != "") {
            ...
            setListuser(null);
            setNotification("Vui lòng nhập tên bạn muốn tìm kiếm");
        }
        setLoading(false);
    }
